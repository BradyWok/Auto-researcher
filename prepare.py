...
EVAL_TOKENS = 2 ** 17   # ~131K tokens for val eval
...
DATASET_URL = "https://huggingface.co/datasets/karpathy/tinystories-gpt4-clean/resolve/main/tinystories_gpt4_clean.parquet"
PARQUET_FILENAME = "tinystories_gpt4_clean.parquet"
VAL_ROW_GROUPS = 50     # last N row groups reserved for validation
...
def download_data():
    """Download the single TinyStories parquet file."""
    ...
    print(f"Data: downloading {PARQUET_FILENAME} (~641 MB)...")
    max_attempts = 5
    ...
    raise RuntimeError(f"Failed to download {PARQUET_FILENAME} after {max_attempts} attempts")
...
def text_iterator(max_chars=200_000_000, doc_cap=10_000):
    """Yield documents from training split (all row groups except last VAL_ROW_GROUPS)."""
    filepath = os.path.join(DATA_DIR, PARQUET_FILENAME)
    pf = pq.ParquetFile(filepath)
    total_rgs = pf.num_row_groups
    train_rgs = total_rgs - VAL_ROW_GROUPS
    nchars = 0
...
def train_tokenizer():
    ...
    if not os.path.exists(filepath):
        print("Tokenizer: data file not found. Run download first.")
        sys.exit(1)
...
def _document_batches(split, tokenizer_batch_size=128):
    """Infinite iterator over document batches, split by row group index."""
    filepath = os.path.join(DATA_DIR, PARQUET_FILENAME)
    ...
    if split == "train":
        rg_indices = list(range(train_rgs))
    else:
        rg_indices = list(range(train_rgs, total_rgs))
